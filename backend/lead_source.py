...
DataForSEO (default, produção — ver ADR-002) vs Scrappa (dev/teste). Resolve UMA
...
`LeadSourceError` é a exceção-base que a rota captura. `ScrappaError` herda de
...
# Exceção-base comum. DataForSEOError já é a raiz (ScrappaError herda dela).
